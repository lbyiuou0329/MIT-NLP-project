...
from src.utils.extraction_utils import get_string_match_subset, get_topic_model_subset, get_dates
...
def get_data():
    ...
    try:
        text = pd.read_csv(args.text_path+"{}{}{}.tsv.gz".format(date.year), sep='\t', usecols=['tweet_id', 'lang', 'tweet_text_clean', 'tweet_text_keywords'])
        if args.lang is not None:
            text = text[text['lang']==args.lang.lower()]
        text = text[text['tweet_text_keywords'].notnull()]

        geo = pd.read_csv(args.geo_path + '{}-{}-{}.tsv.gz'.format(date.year, str(date.month).zfill(2), str(date.day).zfill(2)), sep=',', usecols=['tweet_id', 'seder_id', ''])
